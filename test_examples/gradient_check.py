...
def numerical_check(x, f, df, h, *args):
    ...
    return abs(f_ahead - f_grad), np.sqrt((h**2)*(gradient @ gradient))
...
full_KM = linearKernel(center_trainX)
lambda_ = 1e-8
...
h = 1e-9
err, step = numerical_check(alpha, model.lossFunction, model.lossGradient, h, full_KM, train_y)
print(err, step)