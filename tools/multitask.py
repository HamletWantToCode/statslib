...
class Multi_task_Regressor(BaseRegressor):
    ...
    def lossFunction(self, alpha, sub_KM, sub_X, sub_y, sub_dy, gamma):
        ...
        return (1.0/n_samples)*(loss_on_function + loss_on_gradient.real) + 0.5*self.Lambda_*regular_term
    ...
    def lossGradient(self, alpha, sub_KM, sub_X, sub_y, sub_dy, gamma):
        ...
        derivative_on_gradient = np.sum(np.sum(D_dy_c[:, np.newaxis, :]*dK, axis=2), axis=0)
...
def special_load_data(full_KM, X, y, dy, n_batch=1):
    N = full_KM.shape[0]
    n_elements = N//n_batch
    for i in range(n_batch):
        sub_KM, sub_X, sub_y, sub_dy = full_KM[i*n_elements:(i+1)*n_elements], X[i*n_elements:(i+1)*n_elements], y[i*n_elements:(i+1)*n_elements], dy[i*n_elements:(i+1)*n_elements]
        yield (sub_KM, sub_X, sub_y, sub_dy)

class Special_SGD(GradientDescent):
    ...
    def run(self, alpha, function, gradient, full_KM, X, y, dy, gamma):
        ...
        n_epoch = 0
        while True:
            loader = special_load_data(full_KM, X, y, dy, self.nb_)
